...
> Cinematic documentary web experience preserving East African heritage — engineered for 3G networks.

Built a lazy-loading image/video pipeline using the **Intersection Observer API**, reducing initial page weight by **60%** while maintaining smooth 60fps scrolling on mobile devices.

`JavaScript ES6+` `HTML5` `CSS3` `Performance Optimization` `Intersection Observer`
...
`Python` `FastAPI` `HMAC-SHA256` `REST APIs` `Render`
...
`NestJS` `TypeScript` `MongoDB` `Mongoose` `Flutter` `Render`
...
*Building digital tools that work anywhere. ⚡ Based in Kenya 🇰🇪*
